...
function initCarLocks ()
	...
	for k,p in ipairs(players) do
		...
		bindKey ( p, "k", "down", doToggleLocked )
		bindKey ( p, "l", "down", doToggleLights )
	end
...
bindKey ( source, "l", "down", doToggleLights )
...
function doToggleLocked ( source )
	...
	if ( theVehicle ) then
		...
		Err_Msg("Ktoré auto zatvoriť?", source)
	end
end	
...
function doLockVehicle ( source )
	...
	if ( theVehicle ) then
		...
		if ( getElementData ( theVehicle, "cl_vehiclelocked") == true ) then
			...
			outputChatBox("Zamkol auto "..vehiclename,source,0,255,0,true)
...
function doUnlockVehicle ( source )
	...
	local vehiclename = getVehicleName ( theVehicle )
		if ( getElementData ( theVehicle, "cl_vehiclelocked") == false ) then
			...
			setElementData ( theVehicle, "cl_vehiclelocked", false)
			setVehicleLocked ( theVehicle, false )
			outputChatBox("Odomkol auto "..vehiclename,source,0,255,0,true)
			if ( getVehicleController ( theVehicle ) == false ) then
				cl_FlashOnce ( source )
			end
		end
...
function doToggleLights ( thePlayer,vehicle, beep )
	local theVehicle = getPedOccupiedVehicle ( thePlayer )  
	if ( getElementType(source) == "vehicle" ) then
		theVehicle = vehicle
	end
	if ( getElementType(source) == "player" ) then
		theVehicle = getElementData ( vehicle, "cl_ownedvehicle" )
	end
	if ( theVehicle ) then
		-- if he was in one
